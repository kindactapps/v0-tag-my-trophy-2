...
export function AdminGuard({ children }: AdminGuardProps) {
  ...
  console.log("[v0] AdminGuard - loading:", loading, "user:", user, "isAdmin:", isAdmin)
  ...
  if (!user || !user.isAuthenticated || !isAdmin) {
    console.log("[v0] AdminGuard - user not authenticated or not admin, showing login prompt")
    return (
      <div className="min-h-screen bg-[#f5f0e8] flex items-center justify-center p-4">
        <Card className="max-w-md w-full shadow-lg border-0 bg-white">
          <CardHeader className="text-center pb-6">
            ...
            <CardTitle className="text-2xl font-bold text-[#2c2c2c] mb-2">
              {user?.isAuthenticated ? "Admin Access Required" : "Authentication Required"}
            </CardTitle>
          ...
          <CardContent className="space-y-4">
            <Button asChild className="w-full bg-[#c44c3a] hover:bg-[#a63d2e] text-white">
              <Link href="/auth/admin">{user?.isAuthenticated ? "Switch to Admin Account" : "Sign In as Admin"}</Link>
            </Button>
  ...
  console.log("[v0] AdminGuard - admin authenticated, rendering children")
  return <>{children}</>
}
